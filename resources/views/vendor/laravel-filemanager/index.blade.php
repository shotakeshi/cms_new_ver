@php
  $layout = request()->has('iframe') ? 'admin.layouts.iframe' : 'admin.layouts.app';
@endphp
@extends($layout)
@section('title')
...
@push('styles')
  ...
  <link rel="stylesheet" href="{{ asset('vendor/laravel-filemanager/css/lfm.css') }}?v={{ time() }}">
@endpush
@section('content')
  <div class="container-fluid mt-3">
      ...
      <div class="col-lg-3">
        ...
        <div class="card">
          <div class="card-body">
            ...
            $used = $total - $free;
            ?>
            <small class="float-right">{{ round($used / $total * 100, 2) }}%</small>
            <h6 class="mt-0">{{ round($used / 1073741824, 2) . ' GB' }} / {{ round($total / 1073741824, 2) . ' GB' }} Used</h6>
            <div class="progress" style="height: 5px;">
              <div class="progress-bar bg-success" role="progressbar" style="width: {{ round($used / $total * 100, 2) }}%;" aria-valuenow="{{ round($used / $total * 100, 2) }}" aria-valuemin="0" aria-valuemax="100"></div>
            </div>
          </div><!--end card-body-->
        </div><!--end card-->
      </div><!--end col-->
    ...
    <nav class="bg-classic text-white fixed-bottom border-top d-none" id="actions">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-gradient-danger w-100" data-dismiss="modal">{{ trans('laravel-filemanager::lfm.btn-close') }}</button>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <div class="container-fluid">
              <div class="row">
                <div class="col-lg-6">
                  <button type="button" class="btn btn-gradient-primary w-100" data-dismiss="modal">{{ trans('laravel-filemanager::lfm.btn-confirm') }}</button>
                </div>
                <div class="col-lg-6">
                  <button type="button" class="btn btn-gradient-danger w-100" data-dismiss="modal">{{ trans('laravel-filemanager::lfm.btn-close') }}</button>
                </div>
