...
@section('content')
    <div class="container-fluid">
        @include('admin.layouts.partials.page-title-box', ['name' => __('site.permission.edit_title'), 'url' => route('permissions.index')])
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror"
                                       value="{{ old('name', $permission->name) }}" placeholder="{{ __('site.permission.name') }}">
                                @error('name')
                                    ...
                                    <div class="col-md-12">
                                        <select  name="slug[]" class="select2 mb-3 select2-multiple" style="width: 100%" multiple="multiple" data-placeholder="{{ __('site.permission.choose') }}">
                                            @foreach($routeNames as $routeName)
                                                @php
                                                    $selected = in_array($routeName, $permission->slug);
                                                    $disabled = in_array($routeName, $permissionExists) && !$selected;
                                                @endphp
                                                <option value="{{ $routeName }}" @selected($selected) @disabled($disabled)>{{ $routeName }}</option>
                                            @endforeach
                                        </select>
                                        @error('slug')
                                        <div class="form-control-feedback text-danger">
